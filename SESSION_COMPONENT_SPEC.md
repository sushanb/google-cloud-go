...
- Per-Session state: `state`, `activeRPC`, `peerInfo`, `refreshConfig`, `heartbeat*`, `nextRPCID`, `slotDrainedFn` (opaque per-session callback, wired once at construction by `SessionPoolImpl.createSession`), tracer, debug ring buffer.
