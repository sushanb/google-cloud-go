...
**PeakEwma updates are gated by tracker.** `sessionList.RecordVRpcOutcome(sh, e2e, backend, ok)`:

- `transportEwma` is **strictly OK-gated** — non-OK never feeds it. Rationale: its input is `e2e−backend`, and non-OK responses typically leave `backendDur=0`, which would double-attribute the full elapsed time to transport. Java parity: `SessionList.java:181-187`.
- `e2eEwma` is **cost-gated, not OK-gated**. OK responses always feed it. Non-OK responses feed it too, but ONLY when `e2e > afeE2eEwmaSeed` (>1ms). The seed acts as a fast-fail cutoff — sub-seed non-OK responses are almost always fast server-side rejections (INVALID_ARGUMENT, PERMISSION_DENIED); letting those in would let a fast-erroring AFE look artificially cheap and game the picker (original OK-gate concern). Above the seed, the response was slow to fail (hang, timeout, transport stall) and the honest cost belongs in the tracker so the picker steers away. Java parity divergence: Java's LeastLatencyPicker only sees the OK path today; the Go client tightens this to catch slow-failure signatures the OK-gate misses.
...
**`LeastInFlightAfePicker` also consults `e2eEwma` via an outlier gate.** The default (LEAST_IN_FLIGHT) picker scores primarily on `NumOutstanding`, but candidates whose `E2eCost` exceeds `afeLeastInFlightEwmaGateNanos` (default 10ms — 10× seed) receive an additive `afeLeastInFlightEwmaPenalty` (default 1e9) that dominates any realistic in-flight count. Rationale: LEAST_IN_FLIGHT has no memory — a rogue AFE that hangs sporadically (e.g. DeadlineExceeded on ~1 in 20 calls) drops its in-flight count as soon as the failed request completes and looks identical to a healthy peer on the next pick, so fair-share traffic keeps flowing at it. The gate gives LEAST_IN_FLIGHT the memory it lacks. **Never-starve:** the penalty is additive, not exclusionary — if every candidate is above the gate, the least-inflight outlier still wins. Java has no parallel today (Java pools use LeastLatencyPicker in this scenario); the Go client keeps LEAST_IN_FLIGHT as the default and tightens it to catch rogue AFEs.
