...
type SessionPoolImpl struct {
	...
	// debugEnabled mirrors bigtable.ClientConfig.EnableClientDebug via
	// session.Config.EnableDebug → NewSessionPoolImpl. Immutable after
	// construction (no atomic needed). When false, every allocating
	// debug recorder in session_pool_debug.go / session_debug.go /
	// session_pool_scaling.go early-returns before touching a slice /
	// map / ring buffer, and the picker skips the PickDecision.Candidates
	// slice construction. Pure atomic-counter bumps (msgsSent, retries,
	// etc.) are NOT gated — a branch check costs more than the atomic.
	debugEnabled bool
}
