...
func TestMetricsExporting(t *testing.T) {
	...
	pool, err := NewBigtableChannelPool(ctx, poolSize, strategy, dialFunc, time.Now(), WithMeterProvider(provider), WithMetricsReporterConfig(btopt.DefaultMetricsReporterConfig()), WithDirectAccessChecker(NewDisabledDirectAccessChecker(nil, nil)))
	if err != nil {
