...
3. **`OnClosing` / `onGoAway` fires immediately** so the pool pulls the session out of `sessionList` routing structures. This is up to `waitServerCloseGrace` (5 min) earlier than the actual stream close — the whole point of GOAWAY is *early* removal from routing, not synchronous teardown.
