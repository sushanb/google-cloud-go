...
1. **`transport_latencies` OTel histogram** — recorded via `sh.session.RecordTransportOverhead(ctx, "Bigtable."+desc.Method(), d)` → `sessionTracer.recordTransportOverhead` (`session_tracer.go:299-311`). This is a real client-side metric, but hosted on `sessionTracer` (a per-`Session` tracer), NOT the request-path `AttemptTracer`. See **invariant #3** for the full label set and emission contract.
