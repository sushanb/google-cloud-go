...
**`SessionClient` → many `*SessionPoolImpl` is 1:N**, keyed by `{resource, direction}`. The `SessionClient` maintains an internal `pools map[string]*ManagedPool` guarded by `poolsMu`. Each `OpenTable` / `OpenAuthorizedView` / `OpenMaterializedView` call registers up to two entries (read + write; MaterializedView is read-only, no write entry). Keys use fixed prefixes so different resource kinds cannot collide:

| Call | Key(s) written |
|---|---|
| `OpenTable("t")` | `"table:<full-name>:read"`, `"table:<full-name>:write"` |
...
| Materialized view | `btpb.OpenMaterializedViewRequest` | `stub.OpenMaterializedView(ctx)` | `MaterializedViewName`, `AppProfileId`, `Permission = PERMISSION_READ` (MV is read-only — never open with PERMISSION_WRITE) | `open_session.payload.materialized_view_name` |
